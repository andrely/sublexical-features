...
from numpy import round, sum, int
...
def stratified_sample(df, size=None, target_col=None):
    ...
    stratified_counts = zip(target_counts.index.values, round(target_counts.values * size / total).astype(int))
...
def dbpedia_bwords(sample=None):
